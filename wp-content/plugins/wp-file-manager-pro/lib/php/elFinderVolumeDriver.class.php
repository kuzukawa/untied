    ...
    protected static function zipArchiveUnzip($zipPath, $toDir)
    {
        try {
            ...
            if ($zip->open($zipPath) === true) {
                ...
                if (file_exists(FILEMANEGERPROPATH.'classes/fm-mimetypes.php')) {
                    include(FILEMANEGERPROPATH.'classes/fm-mimetypes.php');
                    $fm_types = FM_MimeTypes::getMimeTypes();
                }
                // Check total file size after extraction
                $num = $zip->numFiles;
                $size = 0;
                ...
                for ($i = 0; $i < $num; $i++) {
                    ...
                    // check if is not folder and the mimetype of the file is allowed by wp
                    if(isset($pathInfo['dirname']) && $pathInfo['dirname'] != "." && isset($fm_types) && isset($pathInfo['extension']) && isset($fm_types[$pathInfo['extension']]) && !in_array($fm_types[$pathInfo['extension']], $allowed_types)){
                        throw new Exception(elFinder::ERROR_ARCHIVE_TYPE);
                    }

                    if(isset($pathInfo['basename']) && strpos($pathInfo['basename'],'..') !== false){
                        throw new Exception(elFinder::ERROR_ARCHIVE_TYPE);
                    }
